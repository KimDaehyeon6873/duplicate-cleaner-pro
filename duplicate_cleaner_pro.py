...
# -*- coding: utf-8 -*-
"""
Duplicate Cleaner Pro - 고성능 중복 파일 클리너 (Tkinter GUI)

중복 파일을 바이트 단위로 탐지해 안전하게 휴지통으로 이동시키는 Python 기반 도구입니다.
Windows, macOS, Linux 지원.

- 바이트 단위 정확 비교, 고속 해시(BLAKE3/SHA-256)
- 병렬 처리, 스마트 필터링, 직관적 트리뷰 UI
- 안전/설정/실행 가이드 및 최신 업데이트 사항은 README.md 참고

⚠️ 전체 드라이브 스캔 기능은 안정성 문제로 현재 비활성화되어 있습니다.

자세한 설명, 사용법, 라이선스: README.md 파일 참조
"""
# ────────────────────────────── IMPORTS ────────────────────────────────────
...
# ────────────────────────────── 스킵 디렉토리 ─────────────────────────────────
# TODO: 이 부분은 OS별로 다를 수 있으므로, 사용자 설정으로 변경 가능하게 할 것 
# 각 OS별로 제외할 디렉토리 목록과 확장자를 선정하는데 한계가 있어 최종본에는 작성하지 않음. 
...
# ─────────────────────────── 프로듀서/컨슈머 (드라이브 분할) (GUI를 통해 접근 불가) ────────────────
# 아래 코드는 전체 드라이브 스캔 최적화 및 병렬 처리를 위해 설계·구현하였으나,
# 시스템 파일 보호, 안정성, 평가 환경 보호 등을 위해 실제 제출본에서는 비활성화 처리합니다.
# 설계적 고민 및 기술적 시도는 코드, 주석, 보고서에 남깁니다.
...
class DuplicateCleaner(tk.Tk):
    ...
    def __init__(self):
        ...
        # ── 상태 변수
        self.target_dirs: list[str] = []             # 다중 폴더 경로 리스트
        ...
        self.after(0, self._resize_dir_listbox_and_window)
    ...
    def _build_widgets(self):
        ...
        self.dir_listbox = tk.Listbox(top, height=2, width=55, selectmode='extended')
        self.dir_listbox.pack(side='left', padx=5)
        ttk.Button(top, text="폴더 추가", command=self.add_folder).pack(side='left', padx=(0,5))
        ttk.Button(top, text="선택 삭제", command=self.remove_selected_folder).pack(side='left')
        self._resize_dir_listbox_and_window()
    ...
    def _resize_dir_listbox_and_window(self):
        """
        폴더 개수에 따라 Listbox의 높이와, 창 전체의 세로 크기를 자동으로 조정합니다.
        폴더가 많아도 항상 Listbox와 창이 잘리지 않게 보장합니다.
        """
        min_lines = 1
        max_lines = 5  # (원하는 만큼 조절)
        count = max(min_lines, min(max_lines, len(self.target_dirs)))
        self.dir_listbox.config(height=count)
        self.update_idletasks()

        # Listbox의 실제 위치 및 높이 계산
        listbox_y = self.dir_listbox.winfo_rooty() - self.winfo_rooty()
        listbox_h = self.dir_listbox.winfo_height()
        win_width = self.winfo_width()
        win_height = self.winfo_height()
        # 아래 여백(트리뷰, 버튼 등) 고려 (실험적으로 350~400, 본인 UI 맞게 조정)
        extra_height = 390
        desired_bottom = listbox_y + listbox_h + extra_height
        min_h = 480
        max_h = 1000
        new_height = max(min_h, min(desired_bottom, max_h))
        # 창 크기를 실제로 변경 (가로폭은 유지, 세로만 조절)
        self.geometry(f"{win_width}x{int(new_height)}")
    ...
    def add_folder(self):
        ...
        if dirs and dirs not in self.target_dirs:
            self.target_dirs.append(dirs)
            self.dir_listbox.insert('end', dirs)
            self._resize_dir_listbox_and_window()
    ...
    def start_scan(self):
        ...
        if not self.target_dirs:
            ...
            if not self.target_dirs:
                return
        folders = [Path(p) for p in self.target_dirs if Path(p).is_dir()]
        if not folders:
            messagebox.showerror("오류", "유효한 폴더를 선택하세요.")
            return
        # 루트 폴더 전체 스캔 방지 (각 폴더별 검사)
        for folder in folders:
            if (
                (os.name == "nt" and folder.resolve() == Path(folder.drive + "\\")) or
                (os.name != "nt" and folder.resolve() == Path("/"))
            ):
                messagebox.showwarning(
                    "경고",
                    f"루트 드라이브({folder}) 전체를 스캔하는 것은 위험합니다.\n특정 폴더를 선택해 주세요."
                )
                return
        # 스캔 시작
        self._disable_scan_buttons()
        self.scanning = True
        self._kickoff_threaded_scan(folders)
    ...
    def confirm_full_scan(self):
        messagebox.showwarning("⚠ 경고",
            "이 기능은 현재 사용할 수 없습니다.\n"
            "특히 시스템 드라이브(C:) 전체 스캔은 중대한 오류를 유발할 수 있으니 절대 실행하지 마세요!")
    #     """
    ...
    def reset_application_state(self):
        ...
        self._reset_selection_toggle_state()
        ...
        try:
            ...
            if not marker_file.exists():
                guide_message = (
                    ...
                    "1. [찾아보기] 버튼으로 중복 파일을 찾고 싶은 폴더를 선택하세요.\n"
                    "   (※ 전체 드라이브 스캔 기능은 현재 안전을 위해 비활성화되어 있습니다)\n\n"
                    "2. [스캔] 버튼을 눌러 중복 파일 검색을 시작하세요.\n\n"
                    "3. 결과 목록에서 '첫 파일 제외 선택' 등 다양한 기능으로\n"
                    "   정리할 파일을 손쉽게 선택할 수 있습니다.\n\n"
                    "4. [선택 파일 휴지통 이동] 버튼으로 안전하게 삭제하세요.\n\n"
                    "💡 팁: 메뉴 > 고급 설정에서 세부 옵션(확장자, 크기, 제외 폴더 등)을 자유롭게 변경할 수 있습니다."
                )
    ...
    def _load_settings_from_file(self):
        ...
        if settings_path.exists():
            try:
                ...
                # self.target_dirs는 설정 복원 대상이 아님 (세션 한정)
